...
// ============================================================================
static DatasetExporter g_datasetExporter;
static bool g_datasetInitialized = false;

ConnectionTracker::ConnectionTracker(int fp_id, size_t max_connections)
    : fp_id_(fp_id),
      max_connections_(max_connections)
{
    if (!g_datasetInitialized)
    {
        g_datasetExporter.open("training/dataset.csv");
        g_datasetInitialized = true;
    }
}
...
size_t ConnectionTracker::cleanupStale(std::chrono::seconds timeout) {
    ...
    for (auto it = connections_.begin(); it != connections_.end(); ) {
        ...
        if (age > timeout || it->second.state == ConnectionState::CLOSED)
        {
            ...
            FeatureVector fv =
                extractor.extract(it->second);

            g_datasetExporter.exportSample(
                fv,
                appTypeToString(it->second.app_type));
...
void ConnectionTracker::evictOldest() {
    ...
    FeatureVector fv =
        extractor.extract(oldest->second);

    g_datasetExporter.exportSample(
        fv,
        appTypeToString(oldest->second.app_type));
