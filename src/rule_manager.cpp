...
std::optional<RuleManager::BlockReason> RuleManager::shouldBlock(
    ...
    if (isIPBlocked(src_ip)) {
        return BlockReason{BlockReason::Type::IP_ADDRESS, ipToString(src_ip)};
    }
    
    // Check port
    if (isPortBlocked(dst_port)) {
        return BlockReason{BlockReason::Type::DESTINATION_PORT, std::to_string(dst_port)};
    }
    
    // Check app
    if (isAppBlocked(app)) {
        return BlockReason{BlockReason::Type::APPLICATION, appTypeToString(app)};
    }
    
    // Check domain
    if (!domain.empty() && isDomainBlocked(domain)) {
        return BlockReason{BlockReason::Type::DOMAIN_NAME, domain};
    }
