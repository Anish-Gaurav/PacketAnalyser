...
namespace DPI
{
    ...
    void LoadBalancer::run()
    {
        while (running_)
        {
            ...
            packets_received_++;
            std::cout << "[LB" << lb_id_ << "] num_fps_ = " << num_fps_
            ...
            std::cout << "\n========== LB DEBUG ==========\n";
            std::cout << "lb_id        = " << lb_id_ << '\n';
            std::cout << "fp_index     = " << fp_index << '\n';
            std::cout << "num_fps_     = " << num_fps_ << '\n';
            std::cout << "queue size   = " << fp_queues_.size() << '\n';
            std::cout << "count size   = " << per_fp_counts_.size() << '\n';
            std::cout << "==============================\n";

            if (fp_index < 0 ||
                fp_index >= static_cast<int>(fp_queues_.size()))
            {
                std::cerr << "INVALID fp_index\n";
                return;
            }
            ...
            std::cout << "About to increment counter" << std::endl;

            per_fp_counts_.at(fp_index)++;
