...
namespace DPI
{
                                ...
                                const std::string &output_file)
    {
        ...
        // Wait for completion
        waitForCompletion();
        std::cout << "[DEBUG] waitForCompletion() returned" << std::endl;

        try
        {
            std::cout << "[DEBUG] before sleep" << std::endl;

            std::this_thread::sleep_for(std::chrono::milliseconds(200));

            std::cout << "[DEBUG] after sleep" << std::endl;

            stop();

            std::cout << "[DEBUG] after stop" << std::endl;
        }
